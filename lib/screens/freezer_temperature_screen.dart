...
class FreezerTemperatureScreen extends StatefulWidget {
  const FreezerTemperatureScreen({Key? key}) : super(key: key);

  @override
  _FreezerTemperatureScreenState createState() =>
      _FreezerTemperatureScreenState();
}

class _FreezerTemperatureScreenState extends State<FreezerTemperatureScreen> {
  bool isLoading = true;
  List<Map<String, dynamic>> temperatureRecords = [];
  bool temperatureRecordedToday = false;
  bool? hasFreezer; // null = pas encore défini, true = oui, false = non
  bool showFreezerChoice = false;
  ...
  Future<void> _checkFreezerPreference() async {
    try {
      setState(() {
        isLoading = true;
      });
      ...
      // Vérifier si la préférence congélateur existe
      ...
      if (structureDoc.exists) {
        ...
        if (data.containsKey('hasFreezer')) {
          hasFreezer = data['hasFreezer'] as bool;
          if (hasFreezer == true) {
          ...
          // Première fois, afficher le choix
          setState(() {
            showFreezerChoice = true;
            isLoading = false;
          });
        }
      }
    } catch (e) {
      print("Erreur lors de la vérification de la préférence congélateur: $e");
      setState(() {
        isLoading = false;
      });
    }
  ...
  Future<void> _saveFreezerPreference(bool hasFreezerChoice) async {
    try {
      final structureId = await _getStructureId();
      ...
      await FirebaseFirestore.instance
          .collection('structures')
          .doc(structureId)
          .update({'hasFreezer': hasFreezerChoice});

      setState(() {
        hasFreezer = hasFreezerChoice;
        showFreezerChoice = false;
      });
      ...
      print("Erreur lors de la sauvegarde de la préférence congélateur: $e");
    }
  }
  ...
  Color _getTemperatureColor(double temp) {
    if (temp > -15) {
      return Colors.red; // Trop élevée pour un congélateur
    } else if (temp > -18) {
      return Colors.orange; // Limite haute
    } else if (temp < -22) {
      return Colors.blue; // Très basse
    }
    return Colors.green; // Normale pour un congélateur
  }

  // Widget pour le choix initial
  Widget _buildFreezerChoiceDialog() {
    return Container(
            ...
            child: Column(
              children: [
                ...
                FutureBuilder<String>(
                  future: _getStructureName(),
                  builder: (context, snapshot) {
                    final structureName = snapshot.data ?? "votre MAM";
                    return Text(
                      "Avez-vous un congélateur au sein de $structureName ?",
                      style: TextStyle(
                        fontSize: 16,
                  ...
                  children: [
                    Expanded(
                      child: Material(
                        ...
                        child: InkWell(
                          onTap: () => _saveFreezerPreference(false),
                    ...
                    Expanded(
                      child: Material(
                        color: Colors.transparent,
                        child: InkWell(
                          onTap: () => _saveFreezerPreference(true),
                          borderRadius: BorderRadius.circular(16),
                          child: Container(
                            padding: EdgeInsets.symmetric(vertical: 16),
                            decoration: BoxDecoration(
  ...
  Future<String> _getStructureName() async {
    try {
      final structureId = await _getStructureId();
      if (structureId.isEmpty) return "votre MAM";
      ...
      if (structureDoc.exists) {
        final data = structureDoc.data() ?? {};
        return data['structureName'] ?? "votre MAM";
      }
    } catch (e) {
      print("Erreur lors de la récupération du nom de structure: $e");
    }
    return "votre MAM";
  }
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      "La température du congélateur n'a pas été relevée aujourd'hui",
                      style: TextStyle(
                        color: Colors.red.shade900,
                        fontWeight: FontWeight.w500,
                        fontSize: 16,
      ...
      body: Column(
        children: [
          ...
          isLoading
              ...
              : Expanded(
                  child: showFreezerChoice
                      ? _buildFreezerChoiceDialog()
                      : hasFreezer == false
                          ? Center(
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
