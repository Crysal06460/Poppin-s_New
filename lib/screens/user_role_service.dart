...
enum UserRole { assistanteMat, parent, unknown }
...
class UserRoleService {
  ...
  static Future<UserRole> determineUserRole() async {
    try {
      ...
      final structureDoc =
          await _firestore.collection('structures').doc(currentUser.uid).get();
    ...
    try {
      ...
      switch (userRole) {
        case UserRole.assistanteMat:
          String structureRefId = currentUser.uid;
          DocumentSnapshot structureSnap = await _firestore
          ...
          if (!structureSnap.exists) {
            ...
            final linkedStructureId =
                userDoc.data()?['structureId']?.toString().trim() ?? '';
            if (linkedStructureId.isNotEmpty) {
              structureRefId = linkedStructureId;
              structureSnap = await _firestore
                  .collection('structures')
                  .doc(structureRefId)
                  .get();
            }
