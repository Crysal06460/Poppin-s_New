...
class _ScheduleInfoScreenState extends State<ScheduleInfoScreen> {
  ...
  Future<void> _initialise() async {
    ...
    try {
      ...
      if (user == null && widget.structureIdOverride == null) {
        setState(() {
      ...
      final updater = user?.uid ?? structureId;
  ...
  Future<String> _resolveStructureId(User? user) async {
    if (widget.structureIdOverride != null &&
        widget.structureIdOverride!.isNotEmpty) {
      return widget.structureIdOverride!;
    }
    if (user == null) {
  ...
  void _onParentNavigationTap(int index) {
  ...
  Widget build(BuildContext context) {
    ...
    final childName = widget.childName ?? 'l’enfant';
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Horaires de $childName'),
      ),
      body: _loading
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(
                                  _isParentMode
                                      ? 'Sélectionnez le mode qui correspond à vos besoins familiaux.'
                                      : 'Définissez les horaires de présence pour finaliser l’inscription.',
                                    ...
                                    onPressed: () {
                                      if (_structureId == null) {
                                        ScaffoldMessenger.of(context)
                                          ...
                                          const SnackBar(
                                            content: Text(
                                              'Impossible de continuer sans structure.',
                                            ),
                                          ),
                                        );
                                        return;
      ...
      bottomNavigationBar: _isParentMode
          ? BottomNavigationBar(
              currentIndex: _selectedIndex,
              onTap: _onParentNavigationTap,
              items: const [
