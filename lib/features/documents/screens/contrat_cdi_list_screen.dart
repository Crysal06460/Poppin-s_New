...
import 'package:poppins_app/features/documents/screens/contrat_cdi_wizard_screen.dart';
import 'package:poppins_app/features/documents/screens/contrat_cdi_pdf_viewer_screen.dart';
import 'package:poppins_app/features/documents/widgets/contrat_cdi_card.dart';
...
class ContratCdiListScreen extends StatelessWidget {
  final String userId;

  const ContratCdiListScreen({super.key, required this.userId});

  void _creerNouveauContrat(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => ContratCdiWizardScreen(userId: userId)));
  }
    ...
    try {
      ...
      if (response.statusCode == 200 && context.mounted) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => ContratCdiPdfViewerScreen(pdfBytes: response.bodyBytes, nomEnfant: nomAffiche),
          ),
        );
  ...
  Future<void> _partagerPdf(BuildContext context, Map<String, dynamic> contrat) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final nomFichier = (contrat['nomFichier'] as String?) ?? 'contrat_cdi.pdf';
        await ContratCdiPdfService().partagerPdf(response.bodyBytes, nomFichier);
      }
    } catch (_) {}
  }
  ...
  Future<void> _supprimerContrat(BuildContext context, Map<String, dynamic> contrat) async {
    ...
    if (confirme == true && context.mounted) {
      try {
        await ContratCdiStorageService().supprimerContrat(userId, contrat['id'] as String, contrat['nomFichier'] as String);
      } catch (_) {}
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Contrats CDI', style: TextStyle(color: _primaryBlue, fontWeight: FontWeight.w700, fontSize: 17)),
        actions: [
          Padding(
            padding: const EdgeInsets.only(right: 12),
            child: IconButton(
              icon: const Icon(Icons.add_circle_outline, color: _primaryBlue, size: 28),
              tooltip: 'Nouveau contrat CDI',
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
        stream: ContratCdiStorageService().listerContratsGeneres(userId),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Nouveau CDI', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Aucun contrat CDI', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Color(0xFF2D3436))),
            const SizedBox(height: 8),
            const Text(
              'Créez votre premier contrat CDI d\'assistant maternel agréé en remplissant le formulaire étape par étape.',
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Créer un contrat CDI', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
