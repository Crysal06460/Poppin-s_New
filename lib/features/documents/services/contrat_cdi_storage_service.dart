...
class ContratCdiStorageService {
  ContratCdiStorageService({FirebaseFirestore? firestore, FirebaseStorage? storage})
  ...
  DocumentReference<Map<String, dynamic>> _brouillonRef(String userId) =>
      _firestore.collection('contrats_cdi').doc(userId).collection('brouillons').doc('current');

  CollectionReference<Map<String, dynamic>> _historiqueRef(String userId) =>
      _firestore.collection('contrats_cdi').doc(userId).collection('historique');

  Future<void> sauvegarderBrouillon(String userId, ContratCdiModel model) async {
    final data = model.toJson()..['updatedAt'] = FieldValue.serverTimestamp();
    await _brouillonRef(userId).set(data, SetOptions(merge: true));
  }

  Future<ContratCdiModel?> chargerBrouillon(String userId) async {
    final snapshot = await _brouillonRef(userId).get();
    if (!snapshot.exists || snapshot.data() == null) return null;
    try {
      return ContratCdiModel.fromJson(snapshot.data()!);
    } catch (_) {
      return null;
    }
  }
  ...
  Future<String> sauvegarderPdfGenere(String userId, Uint8List pdfBytes, String nomFichier) async {
    final storageRef = _storage.ref().child('documents').child(userId).child('contrats_cdi').child(nomFichier);
  ...
  Future<void> supprimerContrat(String userId, String docId, String nomFichier) async {
    ...
    try {
      await _storage.ref().child('documents').child(userId).child('contrats_cdi').child(nomFichier).delete();
    } catch (_) {}
  }
