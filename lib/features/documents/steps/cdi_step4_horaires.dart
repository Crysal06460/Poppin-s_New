...
class CdiStep4Horaires extends StatefulWidget {
  final ContratCdiModel model;
  final Function(ContratCdiModel) onUpdate;

  const CdiStep4Horaires({super.key, required this.model, required this.onUpdate});

  @override
  CdiStep4HorairesState createState() => CdiStep4HorairesState();
}

class CdiStep4HorairesState extends State<CdiStep4Horaires> {
  ...
  void initState() {
    ...
    _nbSemainesCtrl = TextEditingController(text: widget.model.nombreSemainesGarde != null ? '${widget.model.nombreSemainesGarde}' : '');
  ...
  bool validate() {
    if (widget.model.typeAccueil == null) {
      _showError('Veuillez sélectionner le type d\'accueil (Cas 1 ou Cas 2)');
      return false;
    }
    if (widget.model.typeAccueil == TypeAccueilCdi.cas2_46semaines) {
      if (widget.model.nombreSemainesGarde == null) {
  ...
  void _ajouterLigne() {
    final nouvelles = List<LignePlanning>.from(widget.model.lignesPlanning)..add(LignePlanning());
  ...
  void _supprimerLigne(int index) {
    ...
    final nouvelles = List<LignePlanning>.from(widget.model.lignesPlanning)..removeAt(index);
    widget.onUpdate(widget.model.copyWith(lignesPlanning: nouvelles));
    setState(() {
      _disposeLigneControllers();
      _jourCtrls = nouvelles.map((l) => TextEditingController(text: l.jourTravail)).toList();
  ...
  void _updateLigne(int index, {String? jour, String? horaires, String? nbHeures}) {
    final lignes = List<LignePlanning>.from(widget.model.lignesPlanning);
  ...
  Widget build(BuildContext context) {
    final isCas2 = widget.model.typeAccueil == TypeAccueilCdi.cas2_46semaines;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Article 4 — Définissez les horaires de garde de l\'enfant', style: TextStyle(fontSize: 14, color: Color(0xFF636E72))),
          const SizedBox(height: 24),

          // ── Cas 1 / Cas 2 ─────────────────────────────────────────────
          _casChoice(TypeAccueilCdi.cas1_52semaines,
            'Cas n°1 — 52 semaines',
            'Y compris les congés (sur 12 mois consécutifs)',
            Icons.calendar_month_outlined,
          ),
          const SizedBox(height: 10),
          _casChoice(TypeAccueilCdi.cas2_46semaines,
            'Cas n°2 — 46 semaines ou moins',
            'Hors congés (sur 12 mois consécutifs)',
            Icons.event_available_outlined,
          ),
          const SizedBox(height: 20),

          // Cas 2 : nombre de semaines
          if (isCas2) ...[
            TextField(
              controller: _nbSemainesCtrl,
              decoration: _dec('Nombre de semaines de garde *', hint: 'ex : 46'),
              keyboardType: TextInputType.number,
              inputFormatters: [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(2)],
              onChanged: (v) => widget.onUpdate(widget.model.copyWith(nombreSemainesGarde: int.tryParse(v))),
            ),
            const SizedBox(height: 20),

            // Option planning par écrit (Cas 2)
            GestureDetector(
              onTap: () {
                setState(() {});
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Icon(widget.model.planningParEcrit ? Icons.check_box : Icons.check_box_outline_blank,
                        color: widget.model.planningParEcrit ? _primaryBlue : const Color(0xFF95A5A6)),
          ...
          if (!widget.model.planningParEcrit) ...[
            if (widget.model.heuresParSemaine != null || widget.model.typeAccueil == TypeAccueilCdi.cas1_52semaines) ...[
              TextField(
                    ...
                    return Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(flex: 4, child: TextField(controller: _horaireCtrls[i], decoration: _dec('', hint: '08h00–17h30'), style: const TextStyle(fontSize: 13), onChanged: (v) => _updateLigne(i, horaires: v))),
                          const SizedBox(width: 8),
          ...
          TextField(
            ...
            decoration: _dec('Délai de prévenance (semaines calendaires)', hint: 'ex : 4'),
            keyboardType: TextInputType.number,
            inputFormatters: [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(2)],
          ...
          if (widget.model.planningParEcrit) ...[
            ...
            TextField(
              ...
              decoration: _dec('Délai de prévenance planning écrit (semaines, min. 2)', hint: 'ex : 8'),
              keyboardType: TextInputType.number,
              inputFormatters: [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(2)],
  ...
  Widget _casChoice(TypeAccueilCdi type, String titre, String sousTitre, IconData icon) {
    final sel = widget.model.typeAccueil == type;
    return GestureDetector(
      onTap: () {
        setState(() {});
        widget.onUpdate(widget.model.copyWith(typeAccueil: type));
      },
