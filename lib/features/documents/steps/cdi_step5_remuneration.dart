...
class CdiStep5Remuneration extends StatefulWidget {
  final ContratCdiModel model;
  final Function(ContratCdiModel) onUpdate;

  const CdiStep5Remuneration({super.key, required this.model, required this.onUpdate});

  @override
  CdiStep5RemunerationState createState() => CdiStep5RemunerationState();
}

class CdiStep5RemunerationState extends State<CdiStep5Remuneration> {
  ...
  // Recalcule à partir d'un modèle déjà mis à jour (évite d'utiliser widget.model périmé).
  void _recalcMensuel() => _recalcFromModel(widget.model);

  void _recalcFromModel(ContratCdiModel m) {
    final brut = m.salaireHoraireBrut;
    final net = m.salaireHoraireNet;
    final heures = m.heuresParSemaine;
    final type = m.typeAccueil;
    final semaines = m.nombreSemainesGarde;

    if (brut != null && heures != null) {
      double? mb;
      double? mn;
      if (type == TypeAccueilCdi.cas1_52semaines) {
        mb = brut * heures * 52 / 12;
        mn = net != null ? net * heures * 52 / 12 : null;
      } else if (type == TypeAccueilCdi.cas2_46semaines && semaines != null) {
        mb = brut * heures * semaines / 12;
        mn = net != null ? net * heures * semaines / 12 : null;
      }
      if (mb != null) {
        final mbRounded = double.parse(mb.toStringAsFixed(2));
        final mnRounded = mn != null ? double.parse(mn.toStringAsFixed(2)) : null;
        setState(() {
          _mensuelBrutCtrl.text = mbRounded.toStringAsFixed(2).replaceAll('.', ',');
          if (mnRounded != null) _mensuelNetCtrl.text = mnRounded.toStringAsFixed(2).replaceAll('.', ',');
        });
        widget.onUpdate(m.copyWith(salaireMensuelBrut: mbRounded, salaireMensuelNet: mnRounded));
        return;
      }
    }
    widget.onUpdate(m);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (widget.model.heuresParSemaine != null && widget.model.typeAccueil != null)
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      widget.model.typeAccueil == TypeAccueilCdi.cas1_52semaines
                          ? 'Formule : brut horaire × ${widget.model.heuresParSemaine}h × 52 ÷ 12'
                          : 'Formule : brut horaire × ${widget.model.heuresParSemaine}h × ${widget.model.nombreSemainesGarde ?? '?'} sem. ÷ 12',
