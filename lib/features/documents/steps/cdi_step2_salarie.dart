...
class CdiStep2Salarie extends StatefulWidget {
  final ContratCdiModel model;
  final Function(ContratCdiModel) onUpdate;

  const CdiStep2Salarie({super.key, required this.model, required this.onUpdate});

  @override
  CdiStep2SalarieState createState() => CdiStep2SalarieState();
}

class CdiStep2SalarieState extends State<CdiStep2Salarie> {
  ...
  void dispose() {
    for (final c in [
      _nomNaissCtrl, _nomUsageCtrl, _prenomCtrl, _adresseCtrl, _villeCtrl,
      _cpCtrl, _telCtrl, _emailCtrl, _secuCtrl, _agremtRefCtrl,
      _rcProNomCtrl, _rcProPoliceCtrl, _autoNomCtrl, _autoPoliceCtrl,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  bool validate() {
    ...
    if ((widget.model.codePostalSalarie ?? '').trim().length != 5) {
      _showError('Le code postal de l\'assistante doit contenir 5 chiffres');
      return false;
    }
  ...
  Future<void> _pickDate(BuildContext ctx, DateTime? current, void Function(DateTime) onPicked) async {
    final picked = await showDatePicker(
      ...
      builder: (ctx2, child) => Theme(
        data: Theme.of(ctx2).copyWith(
          colorScheme: const ColorScheme.light(primary: _primaryBlue, onPrimary: Colors.white),
        ),
        child: child!,
      ),
    );
  ...
  Widget _dateField(String label, DateTime? date, VoidCallback onTap, {bool required = false}) {
    final txt = date != null ? _dateFmt.format(date) : '';
    return GestureDetector(
      onTap: onTap,
      child: AbsorbPointer(
        child: TextField(
          controller: TextEditingController(text: txt),
          decoration: _dec('$label${required ? ' *' : ''}').copyWith(
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            textCapitalization: TextCapitalization.words,
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(nomUsageSalarie: v)),
          ),
          const SizedBox(height: 14),
          ...
          TextField(
            ...
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(adresseSalarie: v)),
          ),
          const SizedBox(height: 14),
          ...
          TextField(
            ...
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(telephoneSalarie: v)),
          ),
          const SizedBox(height: 14),
          ...
          TextField(
            ...
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(emailSalarie: v)),
          ),
          const SizedBox(height: 14),

          TextField(
            controller: _secuCtrl,
            decoration: _dec('N° de Sécurité sociale'),
            keyboardType: TextInputType.number,
            inputFormatters: [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(15)],
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(numeroSecu: v)),
          ),
          ...
          TextField(
            controller: _agremtRefCtrl,
            decoration: _dec('Référence de l\'agrément *'),
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(referenceAgrement: v)),
          ),
          const SizedBox(height: 14),

          _dateField('Date de délivrance', widget.model.dateLivraisonAgrement, () {
            _pickDate(context, widget.model.dateLivraisonAgrement, (d) {
              setState(() {});
          ...
          _dateField('Date de renouvellement', widget.model.dateRenouvellementAgrement, () {
            _pickDate(context, widget.model.dateRenouvellementAgrement, (d) {
              setState(() {});
          ...
          TextField(
            controller: _rcProNomCtrl,
            decoration: _dec('Nom de la compagnie'),
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(assuranceRCProNom: v)),
          ),
          const SizedBox(height: 10),

          TextField(
            controller: _rcProPoliceCtrl,
            decoration: _dec('N° de police'),
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(assuranceRCProPolice: v)),
          ),
          ...
          TextField(
            controller: _autoNomCtrl,
            decoration: _dec('Nom de la compagnie'),
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(assuranceAutoNom: v)),
          ),
          const SizedBox(height: 10),

          TextField(
            controller: _autoPoliceCtrl,
            decoration: _dec('N° de police'),
            onChanged: (v) => widget.onUpdate(widget.model.copyWith(assuranceAutoPolice: v)),
          ),
